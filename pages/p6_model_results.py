...
def show():
    ...
    c2.metric("🎯 Best AUC",       f"{results_df.loc[best,'ROC-AUC']:.4f}")
    c3.metric("📊 Best F1",        f"{results_df.loc[best,'F1']:.4f}")
    c4.metric("✅ Best Accuracy",  f"{results_df.loc[best,'Accuracy']:.4f}")
    c5.metric("📦 Test Set Size",  f"{len(y_te):,} orders")
    ...
    with tab2:
        ...
        perfect, 0.5 is random guessing. Our XGBoost and Random Forest achieve 0.9997
        using only 8 causal variables.
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab3:
        ...
        costly error in supply chain — a disruption you didn't predict means no
        proactive action was taken. <strong>False Positives</strong> cause unnecessary
        escalation but are less costly. Consider adjusting the classification threshold
        to trade precision for recall based on your business priorities.
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab4:
        ...
        err_df.loc[(err_df["True Label"]==1)&(err_df["Predicted"]==0), "Error Type"] = "False Negative"
        err_df.loc[(err_df["True Label"]==0)&(err_df["Predicted"]==1), "Error Type"] = "False Positive"
        ...
        for etype, color in [
            ...
            if len(subset) > 0:
                fig.add_trace(go.Histogram(
                    x=subset, name=f"{etype} ({len(subset):,})",
